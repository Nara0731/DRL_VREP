...
class TwoLink1VrepEnv(vrep_env.VrepEnv):
    ...
    def __init__(
            ...
            server_port=19997,
            scene_path=vrep_scenes_path+'/twolink1.ttt',
                 ):
    ...
    def _make_observation(self):
        ...
        if fF is not None and fF is not 0:
            sin1 = np.sin(lst_o[0]+lst_o[1])
            if np.abs(sin1) < 0.001:
                lst_o.append(0.0)
            else:
                lst_o.append(np.abs(fF[0][2]/sin1))
        else:
            lst_o += [0]
    ...
    def step(self, action):
        ...
        lam2 = 0.07
        lam3 = 0
        # print('vx=%.4f, F=%.4f' % (vx, fz))
        reward = -(lam1*np.linalg.norm(ex) + lam2*np.linalg.norm(ez))

        # Early stop
        target_x = -0.3298 + 0.06    # Initial Point -0.3298
        target_z = 0.0920               # Initial Point  0.0920
        ...
        done2 = (z < 0.07 or z > 0.11 or x < -0.35)
