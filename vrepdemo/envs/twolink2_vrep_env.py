...
class TwoLink2VrepEnv(vrep_env.VrepEnv):
    ...
    def __init__(
            ...
            server_port=19997,
            scene_path=vrep_scenes_path+'/twolink2.ttt',
                 ):
        ...
        self.joints_max_velocity = 10*np.pi/180  # 第二个关节期望0.04rad/s，也就是2.29°
    ...
    def _make_observation(self):
        ...
        if fF is not None and fF is not 0:
            #lst_o.append(np.abs(fF[0][2] * np.sin(lst_o[0]+lst_o[1]-80.19/RAD2DEG)))
            lst_o.append(np.abs(fF[0][2]))
            #print(lst_o[-1])
        else:
            lst_o += [0]
    ...
    def step(self, action):
        ...
        reward = -(lam1*np.linalg.norm(ex) + lam2*np.linalg.norm(ez) + lam3*np.linalg.norm(action))

        # Early stop
        target_x = 0.1702 + 0.06      # Initial Point   0.1702
        target_z = 1.14               # Initial Point   1.0650
        ...
        # print('z',z)
        done1 = (x > target_x) and (ez < 3)
        done2 = (z < 1.05 or z > 1.14)
    ...
    def reset(self):
        ...
        for i in range(50):
            self._make_action(np.zeros(2,))
            self.step_simulation()
        self._make_observation()
        #print('Initial, New Start!!')
