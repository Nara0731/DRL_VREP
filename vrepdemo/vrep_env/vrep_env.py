from vrepdemo.vrep_env import vrep
...
class VrepEnv(gym.Env):
	...
	def __init__(self,server_addr,server_port,scene_path=None):
		...
		self.opM_get1 = vrep.simx_opmode_oneshot
		self.opM_get2 = vrep.simx_opmode_streaming
		self.opM_get3 = vrep.simx_opmode_buffer
	...
	def obj_get_joint_velocity(self, handle):
		velocity = self.RAPI_rc(vrep.simxGetObjectFloatParameter(self.cID, handle,
				   2012,self.opM_get))
		return velocity
	...
	def obj_read_force_sensor1(self, handle):
		state, forceVector, torqueVector = self.RAPI_rc(vrep.simxReadForceSensor(self.cID,handle,
				self.opM_get3))
		if   state & 1 != 1: # bit 0 not set
			return None # sensor data not (yet) available
	...
	def communicationPause(self):
		vrep.simxPauseCommunication(self.cID, True)


	def communicationGoon(self):
